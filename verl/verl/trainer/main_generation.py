...
def main(config):
    ...
    if not isinstance(chat_lst[0], str):
    ...
    if not ray.is_initialized():
        ...
        ray.init(num_gpus=config.trainer.n_gpus_per_node, runtime_env={'env_vars': {'TOKENIZERS_PARALLELISM': 'true', 'NCCL_DEBUG': 'WARN'}})
    ...
    for batch_idx in range(num_batch):
        ...
        if isinstance(batch_chat_lst[0], list):
            # 如果chat是list，则需要使用chat模板
            ...
            # 如果chat是str，则直接使用tokenizer
            inputs = tokenizer.batch_encode_plus(batch_chat_lst,
                                                padding=True,
                                                truncation=True,
                                                max_length=config.rollout.prompt_length,
                                                return_tensors='pt')
